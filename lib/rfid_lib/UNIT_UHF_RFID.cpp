...
String rssi2str(uint8_t num) {
    int decbuff = 256 - num;
    if (num > 0xf) {
        // String hexbuff = String(num, HEX);
        // char* end;
        // strtoimax(hexbuff, &end, 16);
        return ("-" + String(decbuff));
    } else {
        return ("0");
    }
}
...
bool Unit_UHF_RFID::waitMsg(unsigned long time) {
    ...
    if (buffer[0] == 0xaa && buffer[i - 1] == 0xDD) {
        return true;
    } else {
        return false;
    }
...
bool Unit_UHF_RFID::saveCardInfo(CARD *card) {
    // Serial.print("raw rssi: ");Serial.println(buffer[5]);
    // String rssi = hex2str(buffer[5]);
    String rssi = rssi2str(buffer[5]);
